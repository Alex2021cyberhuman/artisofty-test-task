...
###### 3.1. Реализация через HTML-страницы
  ...
  - Если авторизация выполнилась корректно тользователь должен смочь попасть на http://localhost:5000/cabinet. Т.е. этот адрес должен быть закрыт авторизационным атрибутом Authorize.
...
###### 3.2. Реализация через REST API
  ...
  - REST на регистрацию пользователя ожидает на вход модель (LoginRequest) и имеет тип POST:
